...
def publish_signals(stub):
    ...
    print(millis - sampletime)
    sampletime = millis

    source = common_pb2.ClientId(id="app_identifier")
    namespace = common_pb2.NameSpace(name = "Chassi")

    signal = common_pb2.SignalId(name="ECMChasFr03", namespace=namespace)
...
def do_something(network_stub):
    ticker = threading.Event()
    while not ticker.wait(0.010):
       publish_signals(network_stub)

def run():
    channel = grpc.insecure_channel('wicebox-debian:50051')
    ...
    #
    print("-------------- Publish signals with 10 ms--------------")
